...
export default function ActivityItem({ log, accentColor }: Props) {
  ...
  const user = useAppSelector(selectUserById(log.actorId));
  const currentUser = useAppSelector((state) => state.user.currentUser);

  const task = log.taskId ? tasks[log.taskId] : null;
  const destinationColumn = log.toColumnId ? columns[log.toColumnId] : null;

  const taskTitle = log.taskTitle || task?.title || "Tugas";
  const columnTitle = destinationColumn?.title || "Unknown";
  const formattedTime = timeAgo(log.createdAt);

  const isMe = currentUser?.id === log.actorId;
  const displayName = isMe ? "Anda" : user?.name || log.actorName || "User";
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-0.5 sm:gap-0.5">
        ...
        <p className="text-[0.7rem] sm:text-[0.75rem] text-gray-600 leading-snug">
          {log.action === "added" && (
            <>
              Menambahkan tugas <span className="font-medium">“{taskTitle}”</span>
            </>
          )}
          {log.action === "moved" && (
            <>
              Memindahkan tugas <span className="font-medium">“{taskTitle}”</span> ke status{" "}
              <span className="font-medium">{columnTitle}</span>
            </>
          )}
          {log.action === "updated" && (
            <>
              Memperbarui tugas <span className="font-medium">“{taskTitle}”</span>
            </>
          )}
          {log.action === "deleted" && (
            <>
              Menghapus tugas <span className="font-medium">“{taskTitle}”</span>
            </>
          )}
          {!["added", "moved", "updated", "deleted"].includes(log.action) && (
            <>
              Melakukan aktivitas pada <span className="font-medium">“{taskTitle}”</span>
            </>
          )}
        </p>
      </div>
